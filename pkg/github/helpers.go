...
func GetLatestVersionFromGitHub(githubOwner, githubRepo string) (semver.Version, string, error) {
	...
	if err != nil {
		return semver.Version{}, prefix, err
	}
	if text == "" {
		return semver.Version{}, prefix, fmt.Errorf("no version found")
	}
	semVer, err := semver.Make(text)
	return semVer, prefix, err
}

func GetLatestVersionStringFromGitHub(githubOwner, githubRepo string) (string, string, error) {
	...
	if latestVersionString != "" {
		prefix := ""
		versionStr := latestVersionString
		if strings.HasPrefix(latestVersionString, "v") {
			prefix = "v"
			versionStr = strings.TrimPrefix(latestVersionString, "v")

		} else if strings.HasPrefix(latestVersionString, "V") {
			prefix = "V"
			versionStr = strings.TrimPrefix(latestVersionString, "V")
		}

		return versionStr, prefix, nil
	}
...
func GetLatestReleaseFromGitHub(githubOwner, githubRepo string) (string, error) {
	...
	if version == "" || err != nil {
		util.Logger().Warnf("getting latest release using HTTP redirect (%v) - using API instead", err)
...
func GetLatestReleaseFromGitHubURL(gitURL string) (string, error) {
	...
	if !strings.HasPrefix(gitURL, gitHubPrefix) {
		util.Logger().Warnf("cannot determine the latest release of version stream git URL %s\n", gitURL)
		return "", nil
	}
	name := strings.TrimPrefix(gitURL, gitHubPrefix)
	paths := strings.Split(name, "/")
	if len(paths) <= 1 {
		util.Logger().Warnf("cannot parse git URL %s so cannot determine the latest release\n", gitURL)
		return "", nil
	}
	owner := paths[0]
